...
const PassAudit = Form.create()(props => {
    ...
    return (
      <Mutation
        mutation={gql`
          mutation($id: ID!, $status: AuditType!) {
            passAudit(id: $id,status: $status)
          }
        `}
      >
        {mutate => {
          const handleSubmit = e => {
            const passAuditData = {};
            if (e) {
              passAuditData.id = data.id;
              passAuditData.status = e.status;
              try {
                const promise = new Promise(resolve => {
                    // console.log('passAuditData',passAuditData);
                  resolve(mutate({ variables: passAuditData }));
                });
                promise.then(value => {
                  console.log(value);
      ...
      <PageHeaderWrapper>
        <Card bordered={false} title="待处理提现" extra={extraContent}>
          <Query
            ...
            `}
            variables={{ status: 'loading', pageSize: pages, currentPage: cur, type: 'store' }}
          >
